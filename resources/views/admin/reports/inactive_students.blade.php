<x-app-layout>
    <div class="p-8 mx-auto max-w-7xl">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-900">Inactive Students Report</h2>
        </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4 hover:shadow-md transition-shadow duration-200 flex flex-col justify-between">
                <div>
                    <div class="flex items-center gap-3 mb-3">
                        ...
                        <p class="text-xs font-semibold text-slate-800 uppercase tracking-wider">Total Inactive Students</p>
                    </div>
                </div>
                <p class="text-3xl font-bold text-blue-600 mt-2">{{ number_format($metrics['total_inactive'] ?? 0) }}</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 rounded-t-lg">
                <div class="flex items-center gap-2">
                    <form action="{{ route('admin.reports.inactive-students') }}" method="GET" id="perPageForm" class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-3">
                    <form action="{{ route('admin.reports.inactive-students') }}" method="GET" class="flex items-center gap-3">
                            ...
                            <div x-show="open" @click.away="open = false" style="display: none;" class="absolute z-50 mt-2 w-64 rounded-md shadow-lg bg-white border border-gray-200 left-0 origin-top-left">
                                <div class="p-4 space-y-4">
                                    ...
                                    <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                        <a href="{{ route('admin.reports.inactive-students') }}" class="text-sm text-gray-600 hover:text-gray-900 font-medium">Reset</a>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input type="text" name="search" id="search" value="{{ request('search') }}" class="border-gray-300 rounded-md text-sm focus:ring-blue-500 focus:border-blue-500 py-1.5 px-3 w-48 shadow-sm">
                        </div>
                    </form>
                    ...
                    <div x-data="{ open: false }" class="relative z-40">
                        ...
                        <div x-show="open" style="display: none;" class="absolute right-0 mt-2 w-48 bg-gray-800/95 backdrop-blur-sm rounded-xl shadow-xl border border-gray-700/50 overflow-hidden">
                            <div class="py-1">
                                <a href="{{ route('admin.reports.inactive-students.export', 'pdf') }}?{{ http_build_query(request()->all()) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-200 hover:bg-gray-700 hover:text-white transition-colors">
                                ...
                                <a href="{{ route('admin.reports.inactive-students.export', 'excel') }}?{{ http_build_query(request()->all()) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-200 hover:bg-gray-700 hover:text-white transition-colors">
                                ...
                                <a href="{{ route('admin.reports.inactive-students.export', 'csv') }}?{{ http_build_query(request()->all()) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-200 hover:bg-gray-700 hover:text-white transition-colors">
            ...
            <div class="p-4 flex justify-end">
                <div class="text-sm text-gray-600">
                    Total Inactive Students: <span class="font-bold">@if(isset($students) && $students instanceof \Illuminate\Pagination\LengthAwarePaginator) {{ $students->total() }} @else {{ $students->count() }} @endif</span>
                </div>
            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                    No inactive students found matching the criteria.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
