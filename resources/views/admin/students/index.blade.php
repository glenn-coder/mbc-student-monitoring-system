<x-app-layout>
    <div class="p-8 mx-auto max-w-7xl">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-2xl font-bold text-slate-900 tracking-tight">Students</h2>
                <p class="mt-1 text-sm text-slate-500">Manage enrolled students</p>
            </div>
            <a href="{{ route('admin.students.create') }}" class="px-4 py-2 text-sm bg-emerald-600 text-white font-medium rounded-md hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2 flex items-center gap-2 transition-all active:scale-[0.98] shadow-sm hover:shadow-md">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-slate-200">
            <div class="p-4 border-b border-slate-200 flex justify-between items-center bg-slate-50 rounded-t-lg">
                <form action="{{ route('admin.students.index') }}" method="GET" class="flex items-center gap-2" id="perPageForm">
                    ...
                    <span class="text-sm font-medium text-slate-600">of {{ $students->total() }} results</span>
                    @if(request('search'))
                ...
                <form action="{{ route('admin.students.index') }}" method="GET" class="flex items-center gap-3">
                        ...
                        <div x-show="open" @click.away="open = false" style="display: none;" class="absolute z-50 mt-2 w-64 rounded-md shadow-lg bg-white border border-gray-200 left-0 origin-top-left">
                            <div class="p-4 space-y-4">
                                <div>
                                    ...
                                    <select name="course_id" id="course_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                                        <option value="">All Courses</option>
                                        @foreach($courses as $course)
                                        <option value="{{ $course->id }}" {{ request('course_id') == $course->id ? 'selected' : '' }}>{{ $course->code }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                ...
                                <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                    <a href="{{ route('admin.students.index') }}" class="text-sm text-gray-600 hover:text-gray-900 font-medium">Reset</a>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-slate-700">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b border-slate-100 hover:bg-slate-50/50 transition-colors {{ $loop->even ? 'bg-slate-50/30' : '' }}">
                            <td class="px-6 py-4 font-medium text-slate-900 whitespace-nowrap">
                                {{ $student->last_name }}, {{ $student->first_name }}
                            </td>
                            <td class="px-6 py-4 text-slate-600 font-mono text-sm">{{ $student->student_number }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $student->sex }}</td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'view-student-{{ $student->id }}')" title="View" class="inline-flex items-center justify-center w-8 h-8 text-white bg-blue-500 rounded-lg hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 shadow-sm">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    </button>
                                    <a href="{{ route('admin.students.edit', $student) }}" title="Edit" class="inline-flex items-center justify-center w-8 h-8 text-white bg-amber-500 rounded-lg hover:bg-amber-600 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-1 shadow-sm">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                    </a>
                                    <form action="{{ route('admin.students.destroy', $student) }}" method="POST" class="inline-block" id="delete-student-{{ $student->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-delete-student-{{ $student->id }}')" title="Remove" class="inline-flex items-center justify-center w-8 h-8 text-white bg-rose-500 rounded-lg hover:bg-rose-600 transition-colors focus:outline-none focus:ring-2 focus:ring-rose-500 focus:ring-offset-1 shadow-sm">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <x-modal name="confirm-delete-student-{{ $student->id }}" maxWidth="md" focusable>
                                        <div class="p-6 relative text-left">
                                            ...
                                            <div class="flex justify-end gap-3">
                                                ...
                                                <button type="button" onclick="document.getElementById('delete-student-{{ $student->id }}').submit();" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 text-sm font-medium">
                                                    Remove
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <x-modal name="view-student-{{ $student->id }}" maxWidth="2xl" focusable>
                                        <div class="p-6 relative text-left">
                                            <div class="mb-6">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                <div>
                                                    ...
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->first_name }}</span>
                                                </div>
                                                <div>
                                                    <span class="block text-sm font-medium text-gray-500">Middle Name</span>
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->middle_name }}</span>
                                                </div>
                                                <div>
                                                    <span class="block text-sm font-medium text-gray-500">Last Name</span>
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->last_name }}</span>
                                                </div>
                                                <div>
                                                    <span class="block text-sm font-medium text-gray-500">Student Number</span>
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->student_number }}</span>
                                                </div>
                                                <div>
                                                    <span class="block text-sm font-medium text-gray-500">Email</span>
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->email }}</span>
                                                </div>
                                                <div>
                                                    <span class="block text-sm font-medium text-gray-500">Sex</span>
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->sex }}</span>
                                                </div>
                                                <div>
                                                    <span class="block text-sm font-medium text-gray-500">Course</span>
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->course->code ?? 'N/A' }}</span>
                                                </div>
                                                <div>
                                                    <span class="block text-sm font-medium text-gray-500">Year</span>
                                                    <span class="block mt-1 text-base text-gray-900">{{ $student->year }}</span>
                                                </div>
                                                <div>
