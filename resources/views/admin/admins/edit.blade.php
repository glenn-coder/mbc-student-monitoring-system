            ...
            <form action="{{ route('admin.admins.update', $admin) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $admin->name) }}" autocomplete="off" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="username" id="username" value="{{ old('username', $admin->username) }}" autocomplete="off" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" required>
                        @error('username')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $admin->email) }}" autocomplete="off" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        @error('email')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div x-data="{ password: '', showPassword: false, confirmPassword: '', showConfirmPassword: false }" class="md:col-span-2 flex flex-col">
                        <div>
                            <label for="password" class="block text-sm font-bold text-gray-700">New Password <span class="text-xs font-normal text-gray-500">(Leave blank to keep current password)</span></label>
                            <div class="relative mt-1">
                                <input :type="showPassword ? 'text' : 'password'" name="password" id="password" x-model="password" autocomplete="new-password" class="block w-full rounded-md border-gray-300 pr-10 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        ...
                        <div class="mt-6">
                            <label for="password_confirmation" class="block text-sm font-bold text-gray-700">Confirm Password <span class="text-xs font-normal text-gray-500" x-show="password.length === 0">(Leave blank)</span></label>
                            <div class="relative mt-1">
                                <input :type="showConfirmPassword ? 'text' : 'password'" name="password_confirmation" id="password_confirmation" x-model="confirmPassword" autocomplete="new-password" class="block w-full rounded-md border-gray-300 pr-10 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm" :required="password.length > 0">
                        ...
                        <div class="mt-2">
                            ...
                            <div class="mt-2 text-sm space-y-1">
                                <div class="flex items-center gap-2" :class="password === confirmPassword && password.length > 0 ? 'text-blue-600' : 'text-gray-500'">
                                    <svg x-show="password !== confirmPassword || password.length === 0" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                                    </svg>
                                    <svg x-show="password === confirmPassword && password.length > 0" style="display: none;" class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
